...
arg9: seed (optional)\n";
	...
	sprintf(buf, "%s%s_basic-stack_%d-lru_R%s.mrc", argv[2], basename(input_path), k, argv[7]);
	if((wfd = fopen(buf ,"w")) == NULL)
	{ perror("open error for write"); return -1; }

	buf[0] = '\0';
	sprintf(buf, "%s%s_basic-stack_%d-lru_R%s.hist", argv[2], basename(input_path), k, argv[7]);
	...
	if(TIME_FLAG == 1)
		fprintf(stderr, "basic-KRR k= %s rate= %s file= %s TotalTime = %f seconds\n",argv[6],argv[7], basename(input_path) ,tt_time);
